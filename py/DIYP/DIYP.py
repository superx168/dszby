...
# 设置HTTP请求头，伪装成浏览器
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.6261.95 Safari/537.36 QIHU 360EE'}
    try:
...
# 检查响应状态码，判断是否有效
        if response.status_code == 200:
            print(f"{name}: 有效")
            # 将有效的流写入文件
            with open('有效的源.txt', 'a', encoding='utf-8') as f:
                f.write(f"{name},{url}\n")
            return True
        else:
            print(f"{name}: 无效")
...
def read_m3u_file(file_path):
    ...
    # 正则表达式匹配频道名称
            if line.startswith('#EXTINF'):
               match = re.search(r'tvg-name="(.*?)"', line) #'tvg-id="([^"]+)"'
               if match:
                   current_name = match.group(1)  #match.group(1)
...
# 主程序执行
m3u_file_path = 'py/DIYP/live1.m3u'  # M3U文件路径
name_url_pairs = read_m3u_file('py/DIYP/live.m3u')  # 读取并解析M3U文件
