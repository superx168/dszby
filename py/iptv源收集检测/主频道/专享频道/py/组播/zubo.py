...
def multicast_province(config_file):
    ...
    if len(all_ip_ports) != 0:
        ...
        with open(f"py/iptv源收集检测/主频道/专享频道/py/组播/ip/存档_{province}_ip.txt", 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        with open(f"py/iptv源收集检测/主频道/专享频道/py/组播/ip/存档_{province}_ip.txt", 'w', encoding='utf-8') as f:
            f.writelines(lines)    
        ...
        if os.path.exists(template_file):
            with open(template_file, 'r', encoding='utf-8') as f:
                tem_channels = f.read()
            output = [] 
            with open(f"py/iptv源收集检测/主频道/专享频道/py/组播/ip/{province}_ip.txt", 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    ip = line.strip()
                    output.append(f"{province}-组播{line_num},#genre#\n")
                    output.append(tem_channels.replace("ipipip", f"{ip}"))
            with open(f"py/iptv源收集检测/主频道/专享频道/py/组播/扫描/组播_{province}.txt", 'w', encoding='utf-8') as f:
                f.writelines(output)
        else:
...
def main():
    for config_file in glob.glob(os.path.join('ip', 'py/iptv源收集检测/主频道/专享频道/py/组播/*_config.txt')):
    ...
    with open("py/iptv源收集检测/主频道/专享频道/py/组播/zubo_all.txt", "w", encoding="utf-8") as f:
        f.write(f"{current_time}更新,#genre#\n")
        f.write(f"浙江卫视,http://ali-m-l.cztv.com/channels/lantian/channel001/1080p.m3u8\n")
        f.write('\n'.join(file_contents))
    txt_to_m3u("py/iptv源收集检测/主频道/专享频道/py/组播/zubo_all.txt", "py/iptv源收集检测/主频道/专享频道/py/组播/zubo_all.m3u")
    print(f"组播地址获取完成")
