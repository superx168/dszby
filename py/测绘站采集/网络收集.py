...
with open('2.txt', 'r', encoding='utf-8') as file:
    ...
    with open('py/playlist/网络收集.txt', 'w', encoding='utf-8') as outfile:
        for line in lines:
            # 首先检查行是否包含任何提取关键词
...
def parse_file(input_file_path, output_file_name):
    ...
    with open(output_file_name, 'w', encoding='utf-8') as output_file:
        for ip_or_domain, lines in filtered_ip_or_domain_to_lines.items():
            ...
            output_file.write('\n')  # 在每个小段后添加一个空行作为分隔
# 调用函数并传入文件路径和输出文件名
parse_file('py/playlist/网络收集.txt', 'py/playlist/网络收集.txt')
...
# 存储文件路径
file_path = "py/playlist/网络收集.txt"
output_file_path = "py/playlist/网络收集.txt"
...
def merge_and_filter():
    ...
    all_files = [f for f in os.listdir(os.getcwd()) if f.endswith('py/playlist/网络收集.txt')]
...
def remove_duplicates(input_file, output_file):
    ...
    print("去重后的行数：", len(output_lines))
# 使用方法
remove_duplicates('py/playlist/网络收集.txt', 'py/playlist/网络收集.txt')
...
if __name__ == "__main__":
    输入 =  "py/playlist/网络收集.txt"    #input('请输入utf-8编码的直播源文件路径:')
    输出 = "py/playlist/网络收集.txt"
    main(输入, 输出)
...
from pypinyin import lazy_pinyin
# 打开一个utf-8编码的文本文件
with open("py/playlist/网络收集.txt", "r", encoding="utf-8") as file:
    # 读取所有行并存储到列表中
    lines = file.readlines()
# 定义一个函数，用于提取每行的第一个数字
...
with open("py/playlist/网络收集.txt", "w", encoding="utf-8") as file:
